...
from bioos.resource.workflows import WorkflowResource, Workflow
...
class Workspace(metaclass=SingletonType):
    ...
    def basic_info(self) -> dict:
        ...
        :rtype: dict
        """
        workspace_infos = Config.service().list_workspaces({     #这里是BioOsService对象的list_workspaces方法，非对外暴露的全局方法
            "Filter":
                {"IDs": [self._id]}
        }).get("Items")
    ...
    def env_info(self) -> pd.DataFrame:
        ...
        res = []
        for cluster in notebook_env_info.get('Items') + workflow_env_info.get('Items'):
